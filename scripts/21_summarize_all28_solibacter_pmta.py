...
parser = argparse.ArgumentParser(description="Summarize available all28 Solibacter and PmtA batch outputs.")
parser.add_argument("--samples", default="results/sra_accessions_for_solibacter_candidates.txt")
parser.add_argument("--metadata", default="results/solibacter_sample_manifest.tsv")
parser.add_argument("--coverm-dir", default="results/coverm_solibacter_all28")
parser.add_argument("--blastx-dir", default="results/pmta_marker_blastx_all28")
...
parser.add_argument("--out", default="results/all28_solibacter_pmta_combined_summary.csv")
...
if metadata_path.exists():
    for row in read_table(metadata_path, delimiter="\t"):
        metadata[row.get("sample", "")] = row
...
for sample in samples:
    ...
    if coverm_path.exists() and coverm_path.stat().st_size > 0:
        coverm = read_table(coverm_path, delimiter="\t")
    ...
    total_reads = pmta_hits = pmta_rpm = pmta_rpkm = n_markers_hit = top_marker = top_marker_hits = ""
    if r1.exists() and r2.exists() and (b1.exists() or b2.exists()):
        ...
        pmta_hits = sum(counts.values())
        pmta_rpm = pmta_hits / (total_reads / 1_000_000) if total_reads else 0
        pmta_rpkm = pmta_hits / (marker_aa_kb * (total_reads / 1_000_000)) if marker_aa_kb and total_reads else 0
        n_markers_hit = sum(1 for v in counts.values() if v > 0)
        if counts:
            top_marker, top_marker_hits = max(counts.items(), key=lambda item: item[1])

    ratio = ""
    if sol_rel != "" and pmta_rpm != "" and float(sol_rel) > 0:
        ratio = float(pmta_rpm) / float(sol_rel)
    ...
    rows_out.append(
        {
            ...
            "pmta_RPM_per_Solibacter_percent": ratio,
        }
    )
...
print(f"Completed CoverM samples: {sum(1 for r in rows_out if r['solibacter_mapped_reads'] != '')}")
print(f"Completed PmtA samples: {sum(1 for r in rows_out if r['pmta_marker_hit_reads'] != '')}")
